...
import static ru.iteco.fmhandroid.ui.pageObject.EditingNewsPage.NEWS_LIST;
...
public class NewsPage {
    public static final String ACTIVE = Helper.getStringFromResource(R.string.news_control_panel_active);
    public static final String NOT_ACTIVE = Helper.getStringFromResource(R.string.news_control_panel_not_active);
    ...
    public static ViewInteraction EDIT_NEWS_BUTTON = onView(withId(R.id.edit_news_material_button));
    public static ViewInteraction SORT_NEWS_BUTTON = onView(withId(R.id.sort_news_material_button));
    public static ViewInteraction FILTER_NEWS_BUTTON = onView(withId(R.id.filter_news_material_button));
    ...
    public static ViewInteraction FROM_DATE = onView(withId(R.id.news_item_publish_date_start_text_input_edit_text));
    public static ViewInteraction UNTIL_DATE = onView(withId(R.id.news_item_publish_date_end_text_input_edit_text));
    ...
    public void showNewsManagementButton() {
        ...
        EDIT_NEWS_BUTTON.check(matches(allOf(isDisplayed(), isClickable())));
    }

    public void openNewsManagementPage() {
        Allure.step("Отркрыть страницу управления новостями");
        EDIT_NEWS_BUTTON.check(matches(isDisplayed())).perform(click());
    }
    ...
    private void scrollToItem(int position) {
        Allure.step("Прокрутка списка новостей до элемента на позиции " + position);
    ...
    public void clickOnSortingNews() {
        ...
        SORT_NEWS_BUTTON.check(matches(allOf(isDisplayed(), isClickable()))).perform(click());
    }
    ...
    public void openNewsFilter() {
        ...
        FILTER_NEWS_BUTTON.check(matches(isDisplayed())).perform(click());
    }

    public void enterFromWhatDate(int days) {
        Allure.step("Применить фильтр от '" + days + "' даты");
        FROM_DATE.check(matches(isDisplayed())).perform(replaceText(Helper.getDate(days)), closeSoftKeyboard());
    }

    public void enterUntilWhatDate(int days) {
        Allure.step("Применить фильтр до '" + days + "' даты");
        UNTIL_DATE.check(matches(isDisplayed())).perform(replaceText(Helper.getDate(days)), closeSoftKeyboard());
    }

    public void clickOnCheckBoxActive() {
        Allure.step("Заполнить чек-бокс 'Active'");
        ACTIVE_CHECK_BOX.check(matches(isDisplayed())).perform(click());
    }

    public void clickOnCheckBoxNotActive() {
        Allure.step("Заполнить чек-бокс 'Not Active'");
    ...
    public void scrollToNewsItem(int position) {
        Allure.step("Прокрутка к элементу новостей с позицией: " + position);
    ...
    public void checkAllNewsStatus(String expectedStatus) {
        Allure.step("Проверка, что все элементы в списке новостей имеют статус: " + expectedStatus);
    ...
    public void checkAllNewsDateRange(int fromWhatDay, int untilWhatDay) {
        ...
        Allure.step("Проверка, что все элементы в списке новостей имеют дату публикации в интервале от: " + startDate + " до: " + endDate);
        for (int i = 0; i < getNewsItemCount(); i++) {
            scrollToNewsItem(i);
            ...
            assertTrue("Ожидается, что дата для элемента " + i + " будет в интервале от " + startDate + " до " + endDate,
